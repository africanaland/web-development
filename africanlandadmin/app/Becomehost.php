...
class Becomehost extends Model
{
    ...
    protected $fillable = [
        'fname', 'lname', 'email', 'company_name', 'role_id', 'email_token', 'mobile', 'country', 'city', 'status', 'services', 'property_type', 'service_other', 'reply', 'reply_attachment',
    ];
            ...
            if (strpos($services, ",") !== false) {
                ...
                foreach ($explodes as $key => $explode) {
                    ...
                    if ($service_nameObj) {
                        ...
                        $service_name['name'] = "Service Removed";
                    }
                    $array[] = $service_name['name'];
                }
            } else {
                $service_nameObj = Service::where("id", "=", $services)->first();
                if ($service_nameObj) {
                    $service_name = $service_nameObj->toArray();
                } else {
                    $service_name['name'] = "Service Removed";
